...
int main(int argc, char* argv[])
{
  ...
  if (cmdl[{"-h", "--help"}])
  {
    g_printerr("Usage: %s --otsu=[base,st,mt] <filename>\n", argv[0]);
    return 0;
  }

  auto otsu_method = cmdl("otsu", "base").str();
  auto filename = cmdl(1).str();
  if (otsu_method == "base")
    set_otsu_method(BASELINE);
  else if (otsu_method == "st")
    set_otsu_method(ST);
  else if (otsu_method == "mt")
    set_otsu_method(MT);
  else
  {
  ...
  g_debug("Using method: %s", otsu_method.c_str());
  gst_init(&argc, &argv);
  my_code_init();
